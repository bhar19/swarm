...
func (e *Engine) updateContainer(c dockerclient.Container, containers map[string]*Container, full bool) (map[string]*Container, error) {
	...
	if full {
		...
		log.WithFields(log.Fields{"Info config hostconfig": info.Config.HostConfig.BlkioWeight, "Config blkio weight": info.Config.BlkioWeight}).Debugf("Print weights for blkioweight")
		...
		log.WithFields(log.Fields{"container.Config.HostConfig.BlkioWeight" : container.Config.HostConfig.BlkioWeight, "container.Config.BlkioWeight": container.Config.BlkioWeight}).Debugf("Print blkio values in updateContainer function")
		container.Config.HostConfig.BlkioWeight = container.Config.BlkioWeight
